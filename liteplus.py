...
@sql_register("eval",1)
def eval_without_env(expression_to_eval: str):
    """
     Direct python eval a python expression.
     Usage examples:

     select eval(4+3) ;

     You can call also as a dot command like
     .eval 4+3
     When using a dot command, you will have access to set of object to make queries in an imperative manner.

    """
    return direct_eval(repr(expression_to_eval),None)


def direct_eval(expression_to_eval, con: Connection):
    env={}
    if con!=None:
        env["Connection"]=con
    evalResult=eval(expression_to_eval, env)
    return evalResult
...
def process_dot_command(liteCmd,con: Connection):
    """ Internal function used to process
    """
    # sys.stdout.write("dot cmd:"+liteCmd)
    if liteCmd.startswith(".exit"):
        con.close()        
        sys.exit(0)
    elif liteCmd == ".dump":
        sys.stdout.write(
            "\n-- Dump generated by Liteplus \n"
        )
        ...
        sys.stdout.write("\n%s\n" % (liteCmd[6:]))
    elif liteCmd.startswith(".eval "):
        # Chop .eval string and eval
        expression_to_eval=liteCmd[6:]
        eval_result=direct_eval(expression_to_eval, con)
        sys.stdout.write("/** "+expression_to_eval +" Result:" + repr(eval_result) +" **/\n")
    elif liteCmd == ".help":
        showHelp()
    elif liteCmd.startswith("."):
        raise Exception("Unknown command:"+liteCmd+":")

def repl_cycle(con, lite_plus_ver, input_file :TextIO =sys.stdin):
    # REPL cycle
    ...
    while True:
        if input_file ==sys.stdin:
            sys.stdout.write("\n%i-%i>" % (command_index, con.total_changes))        
        current_line = input_file.readline()
        if current_line=="":
            # Readline will return empty string on 
            # end of input: close and quit
            process_dot_command(".exit",con)
        # Tricky: Ignore empty line at start of cycle
        if current_line.strip()=="":
            continue        
        elif current_line.startswith("."):
            process_dot_command(current_line.strip(),con)
        else:
            # Tricky: if it is not a dot command, start a special eval loop
            # Collect buffer
            # Bug if a /* is encountered, the lines must be read up to */
            # But the parser must be able to understand something like
            # select '/*' ; and so on
            # So for the meantime multi line comments with ; inside will generate errors, 
            # Refrain mixing /* ... */to comment statements                    
            buffer = ""    
            buffer += current_line
            ...
            command_index += 1
            if current_line=="":
                # End of input after last sql process:
                process_dot_command(".exit",con)
